...
The skill uses `main_teams.csv` as the default lookup file for Phase 1 match discovery. This file is generated by `generate_teams_csv.py`, a standalone Python script (bundled with this skill, located in `scripts/`).
...
tournaments_csv = "main_leagues.csv"   # input: tournaments to harvest from
teams_csv = "main_teams.csv"           # output: teams with tournament metadata
```

### Output schema (`main_teams.csv`)
...
The script can generate any subset of teams by modifying the input CSV:

- **Use `all_tournaments.csv`**: harvests teams from all available tournaments (large dataset).
- **Use `main_leagues.csv`**: harvests only top-tier leagues (the default for this skill).
- **Create a custom filter**: copy `all_tournaments.csv`, delete unwanted rows, point `tournaments_csv` to your file, and run the script to produce a tailored `main_teams.csv`.
